...
import { Activity, CheckCircle2, Database, DownloadCloud, HardDrive, LoaderCircle, LogOut, RefreshCw, Save, Trash2, TriangleAlert } from 'lucide-react'

import { APIError, type AuthStatus } from '../api/auth'
import { createBackup, deleteBackup, fetchBackups, fetchDetailedHealth, fetchUpdateJob, fetchUpdateStatus, startUpdate, type Backup } from '../api/maintenance'
...
export function HealthDashboard({ status, onLogout, loggingOut, logoutError }: { status: AuthStatus; onLogout: () => void; loggingOut: boolean; logoutError: string }) {
  ...
  const updateJobQuery = useQuery({
    queryKey: ['update-job', updateJobID], queryFn: ({ signal }) => fetchUpdateJob(updateJobID, signal), enabled: updateJobID !== '',
    retry: true, retryDelay: 2500, refetchInterval: (query) => ['completed', 'failed'].includes(query.state.data?.state ?? '') ? false : 2000,
  })
  ...
  async function updateApplication() {
    if (!status.csrf_token || !window.confirm('确定开始在线更新吗？系统将先创建备份，更新期间服务会短暂重启。')) return
    setError(''); setNotice('')
    try {
      const job = await startUpdate(status.csrf_token)
      setUpdateJobID(job.id)
      queryClient.setQueryData(['update-job', job.id], job)
    } catch (reason) { setError(messageFor(reason, '无法启动在线更新')) }
  }
  ...
  return <AppFrame active="health"><main className="main-workspace operations-workspace"><header className="workspace-header operations-header"><div><p className="eyebrow">System readiness</p><h1>健康与备份</h1></div><div className="header-actions"><button className="primary-button" type="button" disabled={creating} onClick={() => void backup()}>{creating ? <LoaderCircle className="is-spinning" size={17} /> : <Save size={17} />} 创建备份</button><button className="secondary-button logout-button" type="button" onClick={onLogout} disabled={loggingOut}><LogOut size={17} />{loggingOut ? '正在退出' : '退出登录'}</button></div></header>
    ...
    <section className="operations-section update-section"><div className="section-commandbar"><div><strong>版本与在线更新</strong><span>仅接受配置仓库中由 GitHub Actions OIDC/Cosign 签名的正式版本。</span></div><button className="icon-command" type="button" title="重新检查版本" aria-label="重新检查版本" onClick={() => void updateStatusQuery.refetch()}><RefreshCw className={updateStatusQuery.isFetching ? 'is-spinning' : ''} size={16} /></button></div>
      ...
      {updateStatusQuery.data && !updateStatusQuery.data.updater_available && <p className="field-hint">未安装宿主机更新助手时只提供版本检测。完整安装步骤见项目文档 <code>docs/online-update.md</code>。</p>}
      {updateJobID && <div className={`update-job state-${updateJobQuery.data?.state ?? 'loading'}`}>{updateJobQuery.isPending ? <LoaderCircle className="is-spinning" size={18} /> : updateJobQuery.data?.state === 'completed' ? <CheckCircle2 size={18} /> : updateJobQuery.data?.state === 'failed' ? <TriangleAlert size={18} /> : <LoaderCircle className="is-spinning" size={18} />}<div><strong>{updateJobQuery.data?.message || '正在重新连接更新助手'}</strong><span>{updateJobQuery.data?.error || (updateJobQuery.data?.version ? `目标版本 ${updateJobQuery.data.version}` : '应用重启期间页面会自动继续轮询')}</span></div></div>}
    </section>
