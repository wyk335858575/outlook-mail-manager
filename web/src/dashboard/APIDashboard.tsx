...
import { BookOpen, Check, Clipboard, KeyRound, LoaderCircle, LogOut, Plus, ShieldCheck, ShieldOff, X } from 'lucide-react'

import { fetchAccounts } from '../api/accounts'
import { createAPIToken, fetchAPITokens, revokeAPIToken, type APIScope, type CreatedAPIToken } from '../api/apiTokens'
...
export function APIDashboard({ status, onLogout, loggingOut, logoutError }: {
  ...
  return <AppFrame active="api">
    <main className="main-workspace operations-workspace">
      ...
      <section className="api-summary-band"><div><span className="overview-icon"><KeyRound size={22} /></span><strong>{active}</strong><small>有效 token</small></div><div><strong>{tokens.length - active}</strong><small>已撤销或过期</small></div><p>外部 API 仅提供读取能力；每个 token 必须同时具备明确 scope 和账号或分组范围。</p></section>
    ...
    {created && <SecretDialog token={created} onClose={() => setCreated(null)} />}
  </AppFrame>
}

function APITutorial({ accounts }: { accounts: Awaited<ReturnType<typeof fetchAccounts>>['accounts'] }) {
  const [account, setAccount] = useState('')
  const [after, setAfter] = useState(() => new Date(Date.now() - 5 * 60_000).toISOString())
  ...
  return <section className="operations-section api-guide" aria-labelledby="api-guide-title">
    <div className="section-commandbar"><div><strong id="api-guide-title">API 使用教程</strong><span>从创建只读 token 到调用最新验证码接口。</span></div><BookOpen size={19} /></div>
    ...
    <div className="api-guide-section api-guide-steps">
      ...
      <ol>
        ...
        <li><strong>3</strong><span>绑定允许访问的账号或分组。未在范围内的账号会与不存在的账号一样返回 404。</span></li>
        <li><strong>4</strong><span>创建后立即将完整 token 保存到服务端环境变量 <code>OMM_API_TOKEN</code>，请求使用 <code>Authorization: Bearer &lt;token&gt;</code>。</span></li>
      </ol>
      <div className="api-security-note"><ShieldCheck size={18} /><div><strong>token 只用于服务端调用</strong><span>不要保存到浏览器前端代码、localStorage、截图或公开仓库。泄露后立即撤销并重新创建。</span></div></div>
    </div>
    ...
    <div className="api-guide-section">
      ...
      <div className="api-example-inputs">
        ...
        <label><span><code>after</code> (RFC 3339 UTC)</span><input value={after} onChange={(event) => setAfter(event.target.value)} spellCheck={false} /></label>
      ...
      <p className="api-parameter-note"><code>wait_seconds=30</code> 会将该账号加入高优先级同步并最多等待 30 秒；<code>after</code> 必须是 UTC 的 RFC 3339 时间，只返回该时间之后的验证码。</p>
    ...
    <div className="api-guide-section api-reference-grid">
      <div><h2>响应字段</h2><dl className="api-field-list"><dt><code>code</code></dt><dd>提取到的验证码；无新验证码时为 <code>null</code>。</dd><dt><code>fresh</code></dt><dd>本次等待期内账号同步成功且找到 <code>after</code> 之后的邮件时为 <code>true</code>。</dd><dt><code>received_at</code></dt><dd>邮件在 Microsoft 端的收件时间。</dd><dt><code>synced_at</code></dt><dd>该账号最后一次成功写入本地索引的时间。</dd><dt><code>account_status</code></dt><dd>账号当前授权/同步状态。</dd><dt><code>retry_after_seconds</code></dt><dd>同步失败或受限流时建议等待的秒数。</dd></dl></div>
      <div><h2>常见问题</h2><dl className="api-field-list"><dt>401</dt><dd>Bearer token 缺失、错误、已撤销或已过期。</dd><dt>404</dt><dd>账号不存在，或 token 没有该账号/分组的访问权。</dd><dt>429</dt><dd>请求过快；降低调用频率并按 <code>Retry-After</code> 重试。</dd><dt>无新验证码</dt><dd><code>code</code> 为 <code>null</code> 是正常空结果。检查 <code>after</code>、发件人和主题条件，不会回退返回旧码。</dd><dt>账号同步失败</dt><dd>查看 <code>account_status</code> 和 <code>retry_after_seconds</code>；如为 <code>reauth_required</code>，先在账号管理重新授权。</dd></dl></div>
    </div>
  </section>
}
...
function TokenDialog({ accounts, csrfToken, onClose, onCreated }: { accounts: Awaited<ReturnType<typeof fetchAccounts>>['accounts']; csrfToken: string; onClose: () => void; onCreated: (token: CreatedAPIToken) => Promise<void> }) {
  ...
  return <div className="dialog-backdrop"><section className="dialog-panel operation-dialog token-dialog" role="dialog" aria-modal="true" aria-labelledby="token-title"><div className="dialog-heading"><div><p className="eyebrow">Scoped access</p><h2 id="token-title">创建 API token</h2></div><button className="field-icon-button" type="button" title="关闭" aria-label="关闭" onClick={onClose}><X size={18} /></button></div>
    <div className="form-grid two-columns"><label className="wide-field"><span>名称</span><input value={name} onChange={(event) => setName(event.target.value)} /></label><fieldset className="wide-field checkbox-group"><legend>只读权限</legend>{scopes.map((scope) => <label key={scope.id}><input type="checkbox" checked={selectedScopes.includes(scope.id)} onChange={(event) => setSelectedScopes((current) => event.target.checked ? [...current, scope.id] : current.filter((item) => item !== scope.id))} /><span>{scope.label}</span><code>{scope.id}</code></label>)}</fieldset><fieldset className="wide-field checkbox-group account-scope-group"><legend>账号范围</legend>{accounts.length === 0 ? <span className="muted-value">尚无账号</span> : accounts.map((account) => <label key={account.public_id}><input type="checkbox" checked={selectedAccounts.includes(account.public_id)} onChange={(event) => setSelectedAccounts((current) => event.target.checked ? [...current, account.public_id] : current.filter((item) => item !== account.public_id))} /><span>{account.display_name || account.primary_email || account.imported_email}</span></label>)}</fieldset><label><span>分组范围（逗号分隔）</span><input list="api-groups" value={groups} onChange={(event) => setGroups(event.target.value)} /><datalist id="api-groups">{availableGroups.map((group) => <option key={group} value={group} />)}</datalist></label><label><span>到期时间（可选）</span><input type="datetime-local" value={expires} onChange={(event) => setExpires(event.target.value)} /></label><label className="wide-field"><span>允许的 IP/CIDR（每行一个，可选）</span><textarea rows={3} value={cidrs} onChange={(event) => setCIDRs(event.target.value)} placeholder="203.0.113.10/32" /></label></div>
    {error && <p className="form-error">{error}</p>}<div className="form-actions dialog-actions"><button className="secondary-button" type="button" onClick={onClose}>取消</button><button className="primary-button" type="button" disabled={saving || !name.trim() || selectedScopes.length === 0 || selectedAccounts.length + selectedGroups.length === 0} onClick={() => void save()}>{saving ? <LoaderCircle className="is-spinning" size={16} /> : <Check size={16} />} 创建 token</button></div>
  </section></div>
}

function SecretDialog({ token, onClose }: { token: CreatedAPIToken; onClose: () => void }) {
  const [copied, setCopied] = useState(false)
  async function copy() { try { await navigator.clipboard.writeText(token.secret); setCopied(true) } catch { setCopied(false) } }
  return <div className="dialog-backdrop"><section className="dialog-panel secret-dialog" role="dialog" aria-modal="true" aria-labelledby="secret-title"><div className="dialog-heading"><div><p className="eyebrow">Shown once</p><h2 id="secret-title">保存 API token</h2></div></div><p className="dialog-copy">关闭后无法再次查看完整 token。丢失时请撤销并重新创建。</p><code className="secret-value">{token.secret}</code><div className="form-actions dialog-actions"><button className="secondary-button" type="button" onClick={() => void copy()}>{copied ? <Check size={16} /> : <Clipboard size={16} />}{copied ? '已复制' : '复制 token'}</button><button className="primary-button" type="button" onClick={onClose}>我已保存</button></div></section></div>
}
