...
export function accountAvatarInitial(account: Pick<Account, 'imported_email' | 'primary_email' | 'display_name'>) {
  const identity = accountIdentityDisplay(account)
  return (identity.primaryEmail || '?').slice(0, 1).toUpperCase()
}
  ...
  return (
    <div className={`account-row status-${account.status}`}>
      ...
      <div className="account-identity">
        <span className="account-avatar" aria-hidden="true">{accountAvatarInitial(account)}</span>
