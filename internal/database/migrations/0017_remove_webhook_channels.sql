...
WHERE channel_id IN (
    SELECT id FROM notification_channels WHERE kind = 'webhook'
);
...
DELETE FROM notification_channels WHERE kind = 'webhook';
...
ALTER TABLE notification_channels RENAME TO notification_channels_v16;
CREATE TABLE notification_channels (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    public_id TEXT NOT NULL UNIQUE,
    name TEXT NOT NULL,
    kind TEXT NOT NULL CHECK (kind IN ('telegram', 'pushplus', 'wxpush')),
...
FROM notification_channels_v16;

ALTER TABLE notification_rules RENAME TO notification_rules_v16;
...
INSERT INTO notification_rules (
    id, public_id, channel_id, name, enabled, personal_inbox, account_public_ids_json,
...
SELECT id, public_id, channel_id, name, enabled, personal_inbox, account_public_ids_json,
    group_names_json, tag_names_json, categories_json, sender_address, sender_domain,
    subject_keywords_json, start_minute, end_minute, require_otp, include_otp,
    created_at_utc, updated_at_utc
FROM notification_rules_v16;

ALTER TABLE notification_deliveries RENAME TO notification_deliveries_v16;
...
SELECT id, public_id, message_id, rule_id, channel_id, event_type, status, dedupe_key,
    payload_json, attempt_count, next_retry_at_utc, last_error, sent_at_utc,
    created_at_utc, updated_at_utc
FROM notification_deliveries_v16;

DROP TABLE notification_deliveries_v16;
DROP TABLE notification_rules_v16;
DROP TABLE notification_channels_v16;
