...
func TestOpenMigratesPersonalRulesAndSyncSecondsFromVersion11(t *testing.T) {
	...
	if err := store.DB.QueryRow(`SELECT COUNT(*) FROM notification_rules WHERE public_id = 'rule_migration'`).Scan(&remainingNotificationRules); err != nil {
		t.Fatalf("query removed webhook rule: %v", err)
	}
	...
	if copied != 1 || remainingNotificationRules != 0 || seconds != 5 {
		t.Fatalf("migration result = copied:%d remaining_notification_rules:%d seconds:%d", copied, remainingNotificationRules, seconds)
	}
	if _, err := store.DB.Exec(`
		INSERT INTO notification_channels (
			public_id, name, kind, config_ciphertext, created_at_utc, updated_at_utc
		) VALUES ('channel_wxpush', 'WXPush', 'wxpush', 'sealed', ?, ?)
	`, now, now); err != nil {
		t.Fatalf("insert WXPush channel after migration: %v", err)
	}
	if _, err := store.DB.Exec(`
		INSERT INTO notification_channels (
			public_id, name, kind, config_ciphertext, created_at_utc, updated_at_utc
...
func TestOpenDisablesLegacyWXPushFromVersion17(t *testing.T) {
	dataDir := t.TempDir()
	databasePath := filepath.Join(dataDir, "outlook-manager.db")
	databaseURIPath := filepath.ToSlash(databasePath)
	if filepath.VolumeName(databasePath) != "" && !strings.HasPrefix(databaseURIPath, "/") {
	...
	db, err := sql.Open("sqlite", (&url.URL{Scheme: "file", Path: databaseURIPath}).String())
	if err != nil {
		t.Fatalf("open version 17 database: %v", err)
	}
	if _, err := db.Exec(`CREATE TABLE schema_migrations (
		version INTEGER PRIMARY KEY, name TEXT NOT NULL, applied_at_utc TEXT NOT NULL
	)`); err != nil {
	...
	) VALUES ('channel_legacy_wxpush', 'Legacy WXPush', 'wxpush', 'legacy-ciphertext', 1, 1, ?, ?)`, now, now); err != nil {
		t.Fatalf("insert legacy WXPush channel: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO notification_rules (
		public_id, channel_id, name, enabled, created_at_utc, updated_at_utc
	) VALUES ('rule_legacy_wxpush', (SELECT id FROM notification_channels WHERE public_id = 'channel_legacy_wxpush'), 'Legacy rule', 1, ?, ?)`, now, now); err != nil {
		t.Fatalf("insert legacy WXPush rule: %v", err)
	}
	for _, status := range []string{"queued", "sending", "sent"} {
		...
		) VALUES (?, (SELECT id FROM notification_channels WHERE public_id = 'channel_legacy_wxpush'), 'test', ?, ?, '{}', ?, ?)`,
			"delivery_"+status, status, "dedupe_"+status, now, now); err != nil {
			t.Fatalf("insert %s legacy delivery: %v", status, err)
		}
	}
	if err := db.Close(); err != nil {
	...
	defer store.Close()
	var enabled, systemEnabled, ruleEnabled, personalOnly int
	if err := store.DB.QueryRow(`SELECT enabled, system_enabled FROM notification_channels WHERE public_id = 'channel_legacy_wxpush'`).Scan(&enabled, &systemEnabled); err != nil {
		t.Fatalf("load migrated channel: %v", err)
	}
	if err := store.DB.QueryRow(`SELECT enabled, personal_only FROM notification_rules WHERE public_id = 'rule_legacy_wxpush'`).Scan(&ruleEnabled, &personalOnly); err != nil {
		t.Fatalf("load migrated rule: %v", err)
	}
	if enabled != 0 || systemEnabled != 0 || ruleEnabled != 0 || personalOnly != 0 {
		t.Fatalf("legacy states = channel:%d system:%d rule:%d personal_only:%d", enabled, systemEnabled, ruleEnabled, personalOnly)
	}
	for _, publicID := range []string{"delivery_queued", "delivery_sending"} {
		var status, lastError string
		if err := store.DB.QueryRow(`SELECT status, last_error FROM notification_deliveries WHERE public_id = ?`, publicID).Scan(&status, &lastError); err != nil {
			t.Fatalf("load migrated delivery %s: %v", publicID, err)
		}
		if status != "failed" || lastError != "wxpush_reconfiguration_required" {
			t.Fatalf("migrated delivery %s = %s/%s", publicID, status, lastError)
		}
	}
	var sentStatus string
	if err := store.DB.QueryRow(`SELECT status FROM notification_deliveries WHERE public_id = 'delivery_sent'`).Scan(&sentStatus); err != nil {
		t.Fatalf("load sent history: %v", err)
	}
	if sentStatus != "sent" {
		t.Fatalf("sent delivery status = %q, want sent", sentStatus)
	}
}
