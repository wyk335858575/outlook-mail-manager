...
func New(db *sql.DB, keyring Keyring, options Options) (*Service, error) {
	...
	return &Service{
		...
		pushPlusURL: options.PushPlusURL, ctx: ctx, cancel: cancel, wake: make(chan struct{}, 1),
	}, nil
}
...
func (s *Service) ListRules(ctx context.Context) ([]Rule, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT r.public_id, c.public_id, c.name, r.name, r.enabled, 0,
...
func (s *Service) CreateRule(ctx context.Context, rule Rule) (Rule, error) {
	...
	if _, err := s.db.ExecContext(ctx, `
		INSERT INTO notification_rules (
			public_id, channel_id, name, enabled, personal_inbox, account_public_ids_json, group_names_json,
	...
	`, publicID, channelID, strings.TrimSpace(rule.Name), rule.Enabled, rule.PersonalInbox, values.accounts, values.groups,
...
func (s *Service) UpdateRule(ctx context.Context, publicID string, rule Rule) (Rule, error) {
	...
	result, err := s.db.ExecContext(ctx, `
		UPDATE notification_rules SET channel_id = ?, name = ?, enabled = ?, personal_inbox = ?, account_public_ids_json = ?,
			...
			updated_at_utc = ? WHERE public_id = ?
	`, channelID, strings.TrimSpace(rule.Name), rule.Enabled, rule.PersonalInbox, values.accounts, values.groups, values.tags,
...
func (s *Service) EnqueueMessage(ctx context.Context, messagePublicID string) error {
	...
	for _, rule := range rules {
		...
		payload := mailPayload(message, rule.IncludeOTP)
		if err := s.insertDelivery(ctx, messageID, &rule, rule.ChannelPublicID, "mail",
			"mail:"+messagePublicID+":"+rule.PublicID+":"+rule.ChannelPublicID, payload); err != nil {
			return err
		}
	}
	s.signal()
...
func (s *Service) send(ctx context.Context, delivery queuedDelivery, secret channelSecret) error {
	...
	case "webhook":
		return s.sendWebhook(ctx, delivery, secret)
	default:
...
func (s *Service) sendWebhook(ctx context.Context, delivery queuedDelivery, secret channelSecret) error {
	body, err := json.Marshal(delivery.payload)
	if err != nil {
		return errors.New("webhook_payload_failed")
	}
	timestamp := strconv.FormatInt(s.now().UTC().Unix(), 10)
	mac := hmac.New(sha256.New, []byte(secret.WebhookSecret))
	mac.Write([]byte(timestamp))
	mac.Write([]byte("."))
	mac.Write(body)
	headers := map[string]string{
		"X-Outlook-Manager-Timestamp": timestamp,
		"X-Outlook-Manager-Delivery":  delivery.publicID,
		"X-Outlook-Manager-Signature": "sha256=" + hex.EncodeToString(mac.Sum(nil)),
	}
	if err := s.doJSONBytes(ctx, secret.WebhookURL, body, nil, headers); err != nil {
		return errors.New("webhook_delivery_failed")
	}
	return nil
}
...
func (s *Service) loadMessageContext(ctx context.Context, publicID string) (int64, messageContext, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT m.id, a.public_id, m.category, m.sender_address, m.subject,
			COALESCE(m.verification_code, ''), m.received_at_utc
	...
	`, strings.TrimSpace(publicID)).Scan(&id, &message.AccountPublicID, &message.Category,
		&message.SenderAddress, &message.Subject, &message.VerificationCode, &received)
	if errors.Is(err, sql.ErrNoRows) {
...
func mailPayload(message messageContext, includeOTP bool) deliveryPayload {
	...
	text := "发件人：" + safeText(message.SenderAddress, 160) + "\n主题：" + safeText(message.Subject, 240)
	payload := deliveryPayload{
		EventType: "mail", Title: title, Text: text, Account: message.AccountPublicID,
		Sender: safeText(message.SenderAddress, 160), Subject: safeText(message.Subject, 240),
...
func secretFromInput(input ChannelInput) channelSecret {
	return channelSecret{
		...
		WebhookURL: strings.TrimSpace(input.WebhookURL), WebhookSecret: strings.TrimSpace(input.WebhookSecret),
	}
}
...
func overlaySecret(current *channelSecret, replacement channelSecret) {
	...
	if replacement.WebhookURL != "" {
		current.WebhookURL = replacement.WebhookURL
	}
	if replacement.WebhookSecret != "" {
		current.WebhookSecret = replacement.WebhookSecret
	}
}
...
func validateChannel(input ChannelInput, secret channelSecret) error {
	...
	case "webhook":
		if secret.WebhookSecret == "" || !validWebhookURL(secret.WebhookURL) {
			return ErrInvalidChannel
		}
	default:
		return ErrInvalidChannel
...
func channelDestination(kind string, secret channelSecret) string {
	...
	case "pushplus":
		...
		return "一对一推送"
	case "webhook":
		parsed, err := url.Parse(secret.WebhookURL)
		if err == nil {
			return parsed.Host
		}
	}
...
func scanRules(rows ruleRows) ([]Rule, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.PublicID, &item.ChannelPublicID, &item.ChannelName, &item.Name, &item.Enabled, &item.PersonalInbox,
