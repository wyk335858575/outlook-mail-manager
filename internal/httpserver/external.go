...
func (api *externalAPI) messages(w http.ResponseWriter, r *http.Request) {
	...
	if account := strings.TrimSpace(r.URL.Query().Get("account")); account != "" {
		permitted, err := grant.AllowsAccount(r.Context(), api.db, account)
		if err != nil {
			api.internalError(w, err)
			return
		}
	...
	if !parseExternalTime(w, r.URL.Query().Get("since"), &filter.Since) || !parseExternalTime(w, r.URL.Query().Get("until"), &filter.Until) {
		return
	}
	limit := 50
...
func (api *externalAPI) latestOTP(w http.ResponseWriter, r *http.Request) {
	...
	after, err := time.Parse(time.RFC3339, afterValue)
	if err != nil {
		writeExternalError(w, http.StatusBadRequest, "invalid_after", "after 必须是 RFC 3339 UTC 时间")
		return
	}
	waitSeconds := 0
	...
	query := `
		...
		WHERE a.public_id = ? AND m.verification_code IS NOT NULL AND m.received_at_utc > ?
			AND m.remote_deleted = 0 AND m.hidden_from_inbox = 0
	`
	args := []any{accountPublicID, after.UTC().Format(time.RFC3339Nano)}
...
func (api *externalAPI) authorize(w http.ResponseWriter, r *http.Request, scope string, limit int) (apitoken.Grant, bool) {
	...
	grant, err := api.tokens.Verify(r.Context(), strings.TrimSpace(strings.TrimPrefix(header, "Bearer ")), clientIP(r), scope)
	if err != nil {
		writeExternalError(w, http.StatusUnauthorized, "unauthorized", "访问凭据无效")
		return apitoken.Grant{}, false
	}
...
func parseExternalTime(w http.ResponseWriter, value string, destination **time.Time) bool {
	...
	if err != nil {
		writeExternalError(w, http.StatusBadRequest, "invalid_time", "时间必须使用 RFC 3339 格式")
		return false
	}
	*destination = &parsed
	return true
